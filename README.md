...
запускать не нужно; executable должен быть установлен и доступен backend-процессу через
`KB_GIGACODE_COMMAND`.

В edit-режиме GigaCode получает только read/edit/write-инструменты текущего repository: shell, web,
Git-операции и дочерние агенты отключены. Qwen запускается с `--approval-mode auto-edit`, поэтому
правки файлов подтверждаются автоматически без недоступного в headless-режиме диалога; остальные
действия это разрешение не открывает. Временные ошибки GigaCode повторяются до трёх раз с backoff,
а логи каждой попытки сохраняются отдельно. Ответ и итоговый статус возвращаются в карточку аннотации
