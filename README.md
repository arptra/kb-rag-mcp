...
целиком и обязан вернуть проверяемые `file:line` evidence. Оба представления получают единый
...
- `kb_search_index` — выполняет поиск строго в указанном `index_id`; этот стабильный tool вызывается
  по `next_calls` из `kb_feature_context` и не зависит от имён управляемых search-tools;
...
вызывает `kb_feature_context`, фиксирует `graph_revision`, затем исполняет возвращённые `next_calls`
к `kb_search_index` только для затронутых сервисов и строит план по repository/service с evidence.
После успешной ручной перестройки графа сервер также записывает по сервису документ
`system-graph/<service>.md` в связанный индекс и обновляет только изменившиеся индексы. Поэтому
маршрутизация доступна и через MCP-граф, и через обычный RAG-поиск по тому же snapshot.
