...
1. Call `kb_system_graph` with the user's feature description. Supply `start_service` only when the user or source evidence identifies it. Default to `direction="both"`, `max_hops=2`, `min_confidence="LOW"`, and `include_unresolved=true`. This call reads only the standalone graph snapshot and must report `rag_queried=false`.
...
5. Only after graph routing is complete, execute the returned `next_calls`. They target `kb_search_index` with an exact `index_id` and service-scoped query. The graph itself is never stored in or retrieved from those indexes.
...
- If `graph_revision` changes during the workflow, call `kb_system_graph` again before finalizing the plan.
