...
не найденные страницы целиком, а максимум три короткие выдержки.
...
3. 12 кандидатов ранжируются внутри RAG, но в контекст LLM попадают максимум 3 результата.
4. Одна выдержка ограничена 240 условными токенами, весь поисковый ответ — 800 токенами.
...
6. Полный документ не отправляется автоматически; для деталей используется ограниченный
   `kb_get_chunk` с бюджетом до 600 токенов.
...
В JSON поиска проверяйте `context_token_count`: значение не должно превышать 800. Количество
`results` — не больше трёх, а одинаковый `source_path` не должен повторяться.
...
Если менялись только `KB_SEARCH_EXCERPT_TOKENS`, `KB_SEARCH_CONTEXT_TOKENS`,
`KB_SEARCH_MAX_CHUNKS_PER_DOCUMENT` или `KB_DOCUMENT_CONTEXT_TOKENS`, перестраивать индекс не нужно —
достаточно перезапустить сервер.
...
```bash
export KB_SEARCH_EXCERPT_TOKENS='180'
export KB_SEARCH_CONTEXT_TOKENS='600'
export KB_DOCUMENT_CONTEXT_TOKENS='450'

./scripts/rag-low-context.sh serve
```

Не уменьшайте лимиты вслепую: после каждого изменения сравнивайте Hit@3 и список промахов. Также не
ставьте `top_k=20` на клиенте и не вызывайте `kb_get_document` после каждого поиска — оба действия
снова раздуют контекст LLM.
