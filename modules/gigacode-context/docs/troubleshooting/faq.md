...
**A:** Yes. This GigaCode build is local-only. `setup-gigacode.sh` installs the
PyPI `milvus-lite` package into the project `.venv`. The MCP process starts its
gRPC server on `127.0.0.1:19530` automatically and persists data under
`.runtime/milvus-lite`.

Embeddings are calculated in-process with the checked-in Transformers.js/ONNX
model. Docker, Ollama, external embedding APIs and API keys are not used. See
the [environment variables guide](../getting-started/environment-variables.md)
for the exact local paths.
