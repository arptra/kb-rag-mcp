...
export class AstCodeSplitter implements Splitter {
    ...
    private langchainFallback: any; // Dependency-free character splitter fallback
    ...
    async split(code: string, language: string, filePath?: string): Promise<CodeChunk[]> {
        ...
        if (!langConfig) {
            console.log(`📝 Language ${language} not supported by AST, using local character splitter for: ${filePath || 'unknown'}`);
        ...
        try {
            ...
            if (!tree.rootNode) {
                console.warn(`[ASTSplitter] ⚠️  Failed to parse AST for ${language}, falling back to local character splitting: ${filePath || 'unknown'}`);
                return await this.langchainFallback.split(code, language, filePath);
            }
            ...
            console.warn(`[ASTSplitter] ⚠️  AST splitter failed for ${language}, falling back to local character splitting: ${error}`);
            return await this.langchainFallback.split(code, language, filePath);
        }
    }
