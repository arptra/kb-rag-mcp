...
export interface MilvusConfig {
    ...
    password?: string;
    ssl?: boolean;
    /** Reject cloud address discovery and any non-loopback Milvus endpoint. */
    localOnly?: boolean;
}

export function isLoopbackMilvusAddress(address: string): boolean {
    try {
        const value = address.includes('://') ? address : `tcp://${address}`;
...
export class MilvusVectorDatabase implements VectorDatabase {
    ...
    protected async resolveAddress(): Promise<string> {
        ...
        if (finalConfig.localOnly) {
            if (!finalConfig.address) {
                throw new Error('MILVUS_ADDRESS is required in local-only mode; cloud token discovery is disabled');
            }
            if (!isLoopbackMilvusAddress(finalConfig.address)) {
                throw new Error(`Local-only mode rejected non-loopback Milvus address: ${finalConfig.address}`);
            }
    ...
    async createCollection(collectionName: string, dimension: number, description?: string): Promise<void> {
        ...
        const createCollectionParams = {
            collection_name: collectionName,
            description: description || `GigaCode Context collection: ${collectionName}`,
            fields: schema,
    ...
    async createHybridCollection(collectionName: string, dimension: number, description?: string): Promise<void> {
        ...
        const createCollectionParams = {
            collection_name: collectionName,
            description: description || `Hybrid GigaCode Context collection: ${collectionName}`,
            fields: schema,
            functions: functions,
            consistency_level: 'Strong' as const,
        };
