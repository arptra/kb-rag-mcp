...
import { installLoopbackOnlyNetworkGuard } from "./offline-network.js";
import { MilvusLiteProcess, parseMilvusLiteEndpoint } from "./milvus-lite-process.js";

let milvusLiteProcess: MilvusLiteProcess | undefined;
...
class ContextMcpServer {
    ...
    constructor(config: ContextMcpConfig) {
        ...
        console.log(`[EMBEDDING] Using local model: ${config.embeddingModelPath}`);
        ...
        // Initialize the upstream Context indexing engine.
                ...
                tools: [
                    {
                        ...
                        inputSchema: {
                            ...
                            properties: {
                                ...
                                splitter: {
                                    type: "string",
                                    description: "Code splitter to use: 'ast' for syntax-aware splitting with automatic local fallback, or legacy value 'langchain' for the dependency-free character splitter",
                                    enum: ["ast", "langchain"],
                                    default: "ast"
                                },
                                customExtensions: {
...
async function main() {
    ...
    const endpoint = parseMilvusLiteEndpoint(config.milvusAddress);
    milvusLiteProcess = new MilvusLiteProcess({
        command: config.milvusLiteCommand,
        dataDir: config.milvusLiteDataDir,
        host: endpoint.host,
        port: endpoint.port,
    });
    await milvusLiteProcess.ensureStarted();
...
process.on('exit', () => milvusLiteProcess?.stopImmediately());
