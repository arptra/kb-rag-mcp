...
const SYNC_LOCK_STALE_ENV = "GIGACODE_CONTEXT_SYNC_LOCK_STALE_MS";

function isBackgroundSyncEnabled(): boolean {
    const value = envManager.get("GIGACODE_CONTEXT_BACKGROUND_SYNC");
    if (!value) {
        return true;
    }
    ...
    switch (value.trim().toLowerCase()) {
        ...
        default:
            console.warn(
                `[SYNC-DEBUG] Invalid GIGACODE_CONTEXT_BACKGROUND_SYNC value '${value}'. ` +
...
function getBackgroundSyncIntervalMs(): number {
    const value = envManager.get("GIGACODE_CONTEXT_SYNC_INTERVAL_MS");
    if (!value) {
    ...
    if (!Number.isFinite(intervalMs) || intervalMs < MIN_SYNC_INTERVAL_MS) {
        console.warn(
            `[SYNC-DEBUG] Invalid GIGACODE_CONTEXT_SYNC_INTERVAL_MS value '${value}'. ` +
...
export class SyncManager {
    ...
    private getSyncLockPath(): string {
        const contextHome = process.env.GIGACODE_CONTEXT_HOME || process.env.HOME || os.homedir();
        return path.join(contextHome, ".context", "mcp-sync.lock");
    }
    ...
    public startBackgroundSync(): void {
        ...
        if (!isBackgroundSyncEnabled()) {
            console.log('[SYNC-DEBUG] Background sync is disabled via GIGACODE_CONTEXT_BACKGROUND_SYNC=false.');
            return;
        }
    ...
    /**
     * Read GIGACODE_CONTEXT_TRIGGER_WATCHER. Default ON — the watcher is cheap and only
    ...
    private isTriggerWatcherEnabled(): boolean {
        const v = (envManager.get('GIGACODE_CONTEXT_TRIGGER_WATCHER') ?? '').trim().toLowerCase();
        if (!v) return true;
        if (['1', 'true', 'yes', 'on'].includes(v)) return true;
        if (['0', 'false', 'no', 'off'].includes(v)) return false;
        console.warn(
            `[SYNC-DEBUG] Invalid GIGACODE_CONTEXT_TRIGGER_WATCHER value '${v}'. ` +
     ...
     * GigaCode CLI hooks can touch ~/.context/.sync-trigger
    ...
    private setupTriggerWatcher(): void {
        if (!this.isTriggerWatcherEnabled()) {
            console.log('[SYNC-DEBUG] Trigger watcher disabled via GIGACODE_CONTEXT_TRIGGER_WATCHER');
            return;
        }
        ...
        const contextHome = process.env.GIGACODE_CONTEXT_HOME || process.env.HOME || os.homedir();
        const contextDir = path.join(contextHome, '.context');
