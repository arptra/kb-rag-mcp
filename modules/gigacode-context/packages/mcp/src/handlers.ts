...
export class ToolHandlers {
    ...
    /**
     * Sync indexed codebases from local Milvus collections
     ...
     * - If local snapshot has extra directories (not in Milvus), remove them
     * - If local snapshot is missing directories (exist in Milvus), recover them
     */
    private async syncIndexedCodebasesFromMilvus(): Promise<void> {
        try {
            console.log(`[SYNC-LOCAL] 🔄 Syncing indexed codebases from local Milvus...`);
            ...
            console.log(`[SYNC-LOCAL] 📋 Found ${collections.length} collections in local Milvus`);

            if (collections.length === 0) {
                console.log(`[SYNC-LOCAL] ✅ No collections found. Skipping snapshot deletion to avoid data loss from transient errors.`);
                return;
            }

            const milvusCodebases = new Set<string>();
            ...
            for (const collectionName of collections) {
                try {
                    ...
                    if (!collectionName.startsWith('code_chunks_') && !collectionName.startsWith('hybrid_code_chunks_')) {
                        console.log(`[SYNC-LOCAL] ⏭️  Skipping non-code collection: ${collectionName}`);
                        continue;
                    }

                    codeCollectionsChecked++;
                    console.log(`[SYNC-LOCAL] 🔍 Checking collection: ${collectionName}`);
                    ...
                    try {
                        ...
                        if (description && description.startsWith('codebasePath:')) {
                            ...
                            if (codebasePath.length > 0) {
                                console.log(`[SYNC-LOCAL] 📍 Found codebase path from description: ${codebasePath} in collection: ${collectionName}`);
                                milvusCodebases.add(codebasePath);
                                successfulExtractions++;
                                extracted = true;
                            }
                        }
                    } catch (descError: any) {
                        console.warn(`[SYNC-LOCAL] ⚠️  Failed to get description for collection ${collectionName}:`, descError.message || descError);
                    }

                    // Fallback: query document metadata for old collections without new description format
                    if (!extracted) {
                        console.log(`[SYNC-LOCAL] 🔄 Falling back to query-based extraction for collection: ${collectionName}`);
                        try {
                            ...
                            if (results && results.length > 0) {
                                ...
                                if (metadataStr) {
                                    ...
                                    if (codebasePath && typeof codebasePath === 'string') {
                                        console.log(`[SYNC-LOCAL] 📍 Found codebase path from query: ${codebasePath} in collection: ${collectionName}`);
                                        milvusCodebases.add(codebasePath);
                                        successfulExtractions++;
                                    } else {
                                        console.warn(`[SYNC-LOCAL] ⚠️  No codebasePath found in metadata for collection: ${collectionName}`);
                                    }
                                } else {
                                    console.warn(`[SYNC-LOCAL] ⚠️  No metadata found in collection: ${collectionName}`);
                                }
                            } else {
                                console.log(`[SYNC-LOCAL] ℹ️  Collection ${collectionName} is empty`);
                            }
                        } catch (queryError: any) {
                            console.warn(`[SYNC-LOCAL] ⚠️  Fallback query failed for collection ${collectionName}:`, queryError.message || queryError);
                        }
                    }
                } catch (collectionError: any) {
                    console.warn(`[SYNC-LOCAL] ⚠️  Error checking collection ${collectionName}:`, collectionError.message || collectionError);
                    // Continue with next collection
                }
            }

            console.log(`[SYNC-LOCAL] 📊 Found ${milvusCodebases.size} valid codebases in Milvus (checked ${codeCollectionsChecked} code collections, ${successfulExtractions} successfully extracted)`);
            ...
            if (codeCollectionsChecked > 0 && successfulExtractions === 0) {
                console.warn(`[SYNC-LOCAL] ⚠️  All ${codeCollectionsChecked} code collection extractions failed. Skipping sync to avoid accidental deletion of local codebases.`);
                return;
            }

            // Get current local codebases
            const localCodebases = new Set(this.snapshotManager.getIndexedCodebases());
            console.log(`[SYNC-LOCAL] 📊 Found ${localCodebases.size} local codebases in snapshot`);

            let hasChanges = false;

            // Remove local snapshots that no longer exist in Milvus.
            for (const localCodebase of localCodebases) {
                if (!milvusCodebases.has(localCodebase)) {
                    ...
                    try {
                        ...
                        console.warn(`[SYNC-LOCAL] ⚠️  Failed to delete local merkle snapshot for removed codebase '${localCodebase}':`, error?.message || error);
                    }

                    console.log(`[SYNC-LOCAL] ➖ Removed snapshot entry not present in Milvus: ${localCodebase}`);
                }
            }
            ...
            for (const milvusCodebase of milvusCodebases) {
                if (!localCodebases.has(milvusCodebase)) {
                    const stats = await this.queryCollectionStats(milvusCodebase);
                    if (stats) {
                        this.snapshotManager.setCodebaseIndexed(milvusCodebase, {
                            ...stats,
                            status: 'completed' as const
                        });
                        hasChanges = true;
                        console.log(`[SYNC-LOCAL] ➕ Recovered codebase from Milvus: ${milvusCodebase} (rows=${stats.totalChunks})`);
                    } else {
                        console.log(`[SYNC-LOCAL] ⏭️  Skipped recovery for ${milvusCodebase} (row count unknown or zero)`);
                    }
                }
            }
            ...
            if (hasChanges) {
                this.snapshotManager.saveCodebaseSnapshot();
                console.log(`[SYNC-LOCAL] 💾 Updated snapshot to match local Milvus state`);
            } else {
                console.log(`[SYNC-LOCAL] ✅ Local snapshot already matches Milvus state`);
            }

            console.log(`[SYNC-LOCAL] ✅ Local Milvus sync completed successfully`);
        } catch (error: any) {
            console.error(`[SYNC-LOCAL] ❌ Error syncing codebases from local Milvus:`, error.message || error);
    ...
    public async handleIndexCodebase(args: any) {
        ...
        try {
            // Reconcile the local snapshot with Milvus first.
            await this.syncIndexedCodebasesFromMilvus();
